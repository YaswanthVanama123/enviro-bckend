...
import { MapDistanceRecord, MapDistanceSyncJob, FREQUENCY_MAP, FREQUENCY_REVERSE_MAP, DAY_OF_WEEK_MAP, DAY_OF_WEEK_REVERSE_MAP } from '../models/MapDistance.js';
import CompanyMapping from '../models/CompanyMapping.js';

// Account type detection constants
const ACCOUNT_TYPE_THRESHOLDS = {
  anchorMinRevenue: 200,
  anchorMinRevenueGreenline: 100,
  bread5MaxMiles: 2.5,    // ~5 minutes at 30mph
  bread15MaxMiles: 7.5,   // ~15 minutes at 30mph
  milesPerMinute: 0.5
};
...
function frequencyToNumber(freqStr) {
  ...
  return FREQUENCY_REVERSE_MAP[normalized] ?? 0;
}
...
export const initializeJobStatus = async () => {
  try {
    // Find any running or paused jobs that can be resumed
    ...
    if (interruptedJob) {
      ...
      // Auto-resume the interrupted job
...
async function resumeInterruptedJob(jobId) {
  ...
  // Get remaining customers to process
  let processedIds = job.processedCustomerIds || [];
  let allIds = job.customerIds || [];

  // Handle legacy jobs that don't have customerIds saved
  // Re-fetch the customer list based on job type
  if (allIds.length === 0) {
    console.log('[MapDistance] Legacy job detected - fetching customer list');

    let customers;
    if (job.jobType === 'update_sync') {
      // For update sync, get customers with existing data
      ...
      // For full sync, get all active customers
    ...
    // Save the customerIds for future resume attempts
    await MapDistanceSyncJob.findByIdAndUpdate(jobId, {
      customerIds: allIds
    });
  }

  // Handle legacy jobs that don't have processedCustomerIds saved
  // Check which customers already have records from this sync job
  if (processedIds.length === 0 && job.processedCustomers > 0) {
    ...
    // Get customers that have records created by this sync job
    ...
    // Save the processedCustomerIds for future resume attempts
    await MapDistanceSyncJob.findByIdAndUpdate(jobId, {
  ...
  // Get customer details for remaining customers
  ...
  // Update job status to running and update totalCustomers if needed
  await MapDistanceSyncJob.findByIdAndUpdate(jobId, {
    status: 'running',
  ...
  // Continue sync with remaining customers
...
export const fetchMapDistance = async (req, res) => {
  try {
    ...
    // Create a sync job for this single customer fetch
...
async function runSingleFetchJob(jobId, customerName, customerId) {
  ...
  try {
    ...
    if (result.success && result.data && result.data.length > 0) {
      // If we have a customerId, save records to database
      if (customerId) {
        // Delete existing records for this customer
        await MapDistanceRecord.deleteMany({ customerId });

        // Parse and save new records
      ...
      await MapDistanceSyncJob.findByIdAndUpdate(jobId, {
        ...
        currentCustomerName: null,
        fetchedData: result.data // Store the fetched data for immediate display
      });
...
export const startSync = async (req, res) => {
  try {
    // Check if a job is already running (check database, not just memory)
    const runningJob = await MapDistanceSyncJob.findOne({ status: 'running' }).lean();
    if (runningJob) {
      return res.status(400).json({
        success: false,
    ...
    // Create sync job with customerIds for resume capability
    const syncJob = new MapDistanceSyncJob({
      status: 'running',
      jobType: 'full_sync',
 ...
 * @param {string} jobId - The sync job ID
 * @param {Array} customers - Array of customers to process
 * @param {boolean} isResume - Whether this is resuming an interrupted job
 */
async function runSyncJob(jobId, customers, isResume = false) {
  ...
  // Create a single browser session for all customers (login once)
  const session = new MapDistanceSession();
  activeSyncSession = session;

  try {
    // Initialize session (login and navigate to map distance page)
    ...
    for (let i = 0; i < customers.length; i++) {
      ...
      // Update progress, lastActivityAt, and current customer
      ...
      try {
        // Fetch distance for this customer using the shared session
        const result = await session.fetchForCustomer(customer.name, (progress, message) => {
          // Progress callback - could emit to websocket here
        });

        if (result.success && result.data && result.data.length > 0) {
          // Delete existing records for this customer (replace with fresh data)
          await MapDistanceRecord.deleteMany({ customerId: customer._id });

          // Parse and save new records with numeric enum values
          const records = result.data.map(item => ({
          ...
          // No data or failed - still count as processed
          await MapDistanceSyncJob.findByIdAndUpdate(jobId, {
      ...
      // Smaller delay since we're not re-logging in each time
    ...
    // Mark job as failed if session initialization fails
    await MapDistanceSyncJob.findByIdAndUpdate(jobId, {
      status: 'failed',
      completedAt: new Date(),
      currentCustomerName: null,
...
export const getSyncStatus = async (req, res) => {
  try {
    // Check database for running job (survives server restart)
    const runningJob = await MapDistanceSyncJob.findOne({ status: 'running' }).lean();

    // Check for paused job
    const pausedJob = await MapDistanceSyncJob.findOne({ status: 'paused' }).lean();

    // Get the latest job (could be running, paused, or completed)
    const latestJob = await MapDistanceSyncJob.findOne()
      .sort({ createdAt: -1 })
      .lean();

    // Check if the job is actually being processed in memory
    // If runningJob exists but activeSyncJobId doesn't match, it's an interrupted job
    const isActuallyRunning = runningJob !== null &&
      activeSyncJobId !== null &&
      runningJob._id.toString() === activeSyncJobId.toString();

    // Job is interrupted if running in DB but not in memory
    const isInterrupted = (runningJob !== null && !isActuallyRunning);

    // Job is paused if status is 'paused'
...
export const getStoredRecords = async (req, res) => {
  try {
    ...
    // Sort by distance ascending when filtering by customer, otherwise by date
    ...
    // Map numeric codes back to strings for display
...
export const getCustomersWithData = async (req, res) => {
  try {
    ...
    // Get distinct customer IDs that have records
    const customerIdsWithData = await MapDistanceRecord.distinct('customerId');
...
export const getStats = async (req, res) => {
  try {
    ...
    // Estimate storage size based on average document size
    // MongoDB typically uses ~100-200 bytes per simple document
    let storageSizeBytes = 0;
    let storageSizeFormatted = 'N/A';
    let avgBytesPerRecord = 0;

    try {
      // Try to get collection stats using db.command
      ...
      // Average document size estimation: ~150 bytes per record with our optimized schema
      storageSizeBytes = totalRecords * 150;
    }

    // If still 0, use estimation
...
export const startUpdateSync = async (req, res) => {
  try {
    ...
    // Get customers that already have distance records
    const customerIdsWithData = await MapDistanceRecord.distinct('customerId');
...
export const resumeSync = async (req, res) => {
  try {
    ...
    // Find the job to resume
    let job;
    if (jobId) {
      job = await MapDistanceSyncJob.findById(jobId);
    } else {
      // Find the most recent paused or interrupted job
    ...
    // Check if another job is actively running (different from the one we're resuming)
...
export const deleteAllRecords = async (req, res) => {
  try {
    ...
    // Also delete all sync job history
 ...
 *
 * Request body:
 * - biginCompanyId: string (optional) - Bigin company ID to look up RouteStar customer
 * - routeStarCustomerId: string (optional) - Direct RouteStar customer ID
 * - perVisitRevenue: number (optional) - Per-visit revenue for revenue-based detection
 * - isGreenline: boolean (optional) - Whether this is a Greenline customer
 *
 * Returns recommended account type based on:
 * 1. Revenue (if >= $200 or $100 for Greenline → Anchor)
 * 2. Distance to nearest Anchor (from stored map distance data)
 */
export const detectAccountType = async (req, res) => {
  try {
    ...
    // If biginCompanyId provided, look up the mapped RouteStar customer
    // Note: biginCompanyId from ZohoMapping is actually the Zoho string ID (biginId field)
    ...
    // Check revenue-based account type first
    ...
    // If no RouteStar customer ID, default to Pit
    if (!rsCustomerId) {
      return res.json({
        success: true,
        accountType: 'Pit',
    ...
    // Get distance data for this customer
    // Find the minimum distance to any other customer (which would be an Anchor candidate)
    ...
    // Find the nearest customer (smallest distance > 0)
    ...
    // Determine account type based on distance
    let accountType;
    let confidence = 'high';
    let reason;
